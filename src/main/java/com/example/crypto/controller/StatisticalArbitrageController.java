...
public class StatisticalArbitrageController {
    ...
    @GetMapping("/adf-test")
    public ResponseEntity<?> getAdfTestResults(@RequestParam String timeframe, @RequestParam String exchange) {
        ...
        try {
            ...
            return ResponseEntity.badRequest().body("获取ADF检验结果失败: " + e.getMessage());
        }
    }

    @GetMapping("/kpss-test")
    public ResponseEntity<?> getKpssTestResults(@RequestParam String timeframe, @RequestParam String exchange) {
        ...
        try {
            ...
            return ResponseEntity.badRequest().body("获取KPSS检验结果失败: " + e.getMessage());
        }
    }

    @GetMapping("/hurst-exponent")
    public ResponseEntity<?> getHurstExponentResults(@RequestParam String timeframe, @RequestParam String exchange) {
        ...
        try {
            ...
            return ResponseEntity.badRequest().body("获取Hurst指数结果失败: " + e.getMessage());
        }
    }
    
    @PostMapping("/recalculate")
    public ResponseEntity<?> recalculateIndices(@RequestParam String timeframe, @RequestParam String exchange) {
        ...
        try {
            ...
            return ResponseEntity.ok("统计套利指标重新计算成功");
        } catch (Exception e) {
            logger.error("重新计算统计套利指标失败: error={}", e.getMessage(), e);
            return ResponseEntity.badRequest().body("重新计算统计套利指标失败: " + e.getMessage());
        }
    }
} 